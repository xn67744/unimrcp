...
static size_t write_callback(void* contents, size_t size, size_t nmemb, void* userp) {
	...
	TempMemoryStruct* mem = (TempMemoryStruct*)userp;
	...
	mem->size += realsize;
	mem->data[mem->size] = 0; // Ensure null-termination

	return realsize;
}

static apt_bool_t http_post_request(const char* text, TempMemoryStruct* response_data) {
	CURL* curl;
	CURLcode res;
	curl = curl_easy_init();
	if (curl) {
		curl_easy_setopt(curl, CURLOPT_URL, "http://127.0.0.1:9880");
...
/**static apt_bool_t demo_synth_channel_speak(mrcp_engine_channel_t *channel, mrcp_message_t *request, mrcp_message_t *response)
{
	...
	// 构建 JSON 请求体
	char* post_data = apr_psprintf(channel->pool, "{\"text\":\"%s\",\"text_language\":\"zh\"}", request->body);
	struct MemoryStruct response_data;
	response_data.memory = malloc(1);  // 初始分配一个字节
	response_data.size = 0;

	// 发送 HTTP POST 请求
	if (http_post_request(post_data, &response_data)) {
		// 处理响应，例如存储在文件或内存中，以便读取
		// response_data.memory 包含 WAV 文件数据
		synth_channel->audio_file = fmemopen(response_data.memory, response_data.size, "rb");
	}
...
static apt_bool_t demo_synth_channel_speak(mrcp_engine_channel_t* channel, mrcp_message_t* request, mrcp_message_t* response) {
	demo_synth_channel_t* synth_channel = channel->method_obj;
	if (!request->body.buf) {
		return FALSE;  // 没有提供文本，返回错误
	}

	// 构建 JSON 请求体，发送到 TTS 服务
	char* post_data = apr_psprintf(channel->pool, "{\"text\":\"%s\",\"text_language\":\"zh\"}", request->body.buf);

	// 初始化内存结构
	TempMemoryStruct response_data = { 0 };
	...
	if (http_post_request(post_data, &response_data)) {
		...
		if (response_data.data && response_data.size > 0) {
			// 将接收到的音频数据写入临时文件或内存
			char* audio_file_path = apr_psprintf(channel->pool, "%s/audio_output.wav", channel->engine->dir_layout);
			FILE* file = fopen(audio_file_path, "wb");
			if (file) {
				fwrite(response_data.data, 1, response_data.size, file);
				fclose(file);
				synth_channel->audio_file = fopen(audio_file_path, "rb");
				if (!synth_channel->audio_file) {
					apt_log(SYNTH_LOG_MARK, APT_PRIO_WARNING, "Failed to open audio file " APT_SIDRES_FMT, MRCP_MESSAGE_SIDRES(request));
		...
		apt_log(SYNTH_LOG_MARK, APT_PRIO_WARNING, "Failed to get response from TTS service " APT_SIDRES_FMT, MRCP_MESSAGE_SIDRES(request));
		response->start_line.status_code = MRCP_STATUS_CODE_METHOD_FAILED;
		free(response_data.data);
		return FALSE;
	}
